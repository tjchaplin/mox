...
var readWriteFileStream = require('./utils/readWriteFileStream');

var mox = function(options, sources){
    if(!sources){
        sources = options;
        options = {};
    }
        ...
        run : function(onComplete){
            ...
            try{
                ...
                getAllFilesToParse(sources,function(sources){
                    ...
                    if(options.moxFile)
                        fs.writeFileSync(options.moxFile,JSON.stringify(documentation,null, 4));

                    var htmlFileStream = stringToStream(rendered);
                    if(options.htmlFile)
                        htmlFileStream = htmlFileStream.pipe(readWriteFileStream(options.htmlFile));

                    var markdownStream = htmlFileStream.pipe(hammerdown({type:options.markdown}));
                    if(options.outputFile)
                        markdownStream = markdownStream.pipe(readWriteFileStream(options.outputFile));

                    markdownStream = markdownStream.pipe(tr);

                    if(onComplete instanceof Function)
                        tr.pipe(concat(function(markdown){ onComplete(null,markdown);}));
